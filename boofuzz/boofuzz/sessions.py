...
class Target(object):
    ...
    def __init__(self, connection, procmon=None, procmon_options=None, netmon=None):
        ...
        # the connect to the target, we need to relpace it to local io 
    ...
    def send(self, data):
        ...
        print '3'#num_sent = self._target_connection.send(data)
        num_sent = self._target_connection.send(data)

        print '2'#num_sent = self._target_connection.send(data)
...
class Session(pgraph.Graph):
    ...
    def fuzz(self):
        ...
        #        raw_input('go?')#num_cases_actually_fuzzed += 1
    ...
    def restart_target(self, target):
        ...
        """
        if self.local:
             self.targets[0]=process(self.target_name)
        #sleep(3)
        #gdb.attach(self.targets[0])#=process(self.target_name) 
    ...
    def transmit(self, sock, node, edge):
        ...
        try:
            ...
            if self._check_data_received_each_request:
                ...
                try: # TODO: Remove magic number (10000)
                    if self.local:
                        self.last_recv = self.targets[0].recv(10000, timeout = 1)
                    else:
                    ...
                    #raw_input('crash?')
                    s = os.getcwd() #raw_input('crash?')
                    for i in range(len(self.all_data)):
                        try:
                            print s+'/crash/'+self.target_name+'/'+str(self.crash_id)+'/'+str(i)	 
                            fo = open(s+'/crash/'+str(self.crash_id)+'/'+str(i), 'wb')	 
                        except:
                            os.makedirs(s+'/crash/'+str(self.crash_id)+'/')
                            fo = open(s+'/crash/'+str(self.crash_id)+'/'+str(i), 'wb')	 
                    #f_name = 'crash_{}.txt'.format(self.crash_id)
                    #fo = open(f_name, 'wb')
                        fo.write(self.all_data[i])
                        fo.close()
                    self.crash_id = self.crash_id + 1
